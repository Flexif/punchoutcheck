...
const OciTestTool = () => {
  ...
  const [customParams, setCustomParams] = useState([{ key: '', value: '', id: uuidv4(), prevKey: '' }]);
  ...
  const handleOnBlur = () => {
    if (!validateURL(formData.baseURL)) {
      setErrorMessage('The PunchOut URL must be a valid URL with the HTTP(S) protocol.');
      setTimeout(() => {
        setErrorMessage('');
      }, 6000);
    } else {
    ...
    if (validateURL(baseURL)) {
      try {
        ...
        const updatedCustomParams = Array.from(searchParams.entries())
          .filter(([key]) => !['username', 'password', 'hook_url'].includes(key))
          .map(([key, value]) => ({ key, value, id: uuidv4(), prevKey: key }));

        setCustomParams((prevParams) => {
          if (JSON.stringify(prevParams) !== JSON.stringify(updatedCustomParams)) {
            return updatedCustomParams.length > 0 ? updatedCustomParams : [{ key: '', value: '', id: uuidv4(), prevKey: '' }];
          }
          return prevParams;
        });
      } catch (error) {
...
// Event listener to update the BaseURL
  ...
  }, [formData ,formData.username, formData.password, formData.hookURL, customParams, errorMessage, debouncedUpdateBaseURL]);
...
//fuction to rest all the input fields and set data 
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (result.success && result.OciPunchoutURL) {
                ...
                setSuccessMessage('The OCI PunchOut session will be open in a new window in a few seconds');
                setTimeout(()=>{
                  setSuccessMessage('');
                }, 6000);
  ...
  return (
    <div className={styles.container}>
          ...
          {customParams.map((param, index) => (
            <div key={param.id} className={styles.customInputs}>
              ...
              <div className={styles.btnContainerAR}>
                <CiCircleRemove size={26} className={styles.removeBtn} onClick={() => handleRemoveParam(index)} />
              </div>
            </div>
          ))}
        </div>
      ...
      {sucessMessage ? <div className={styles.successMessage}>{sucessMessage}</div> : <div></div>}
